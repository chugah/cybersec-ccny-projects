...
namespace Microsoft.Quantum.Samples.DatabaseSearch {
    ...
    operation RunRandomSearch() : Unit {
        // Investigate the success probability of classical random search. This corresponds
        // to the case where only the start state is prepared, and  Grover iterations are
        // not performed to amplify the marked subspace.
        let nIterations = 0;

        // Define the size `N` = 2^n of the database to searched in terms of number of qubits
        // `n`.
        let nDatabaseQubits = 3;
        let databaseSize = 2 ^ nDatabaseQubits;

        // Execute the classical random search and verify that the success probability
    ...
    operation RunQuantumSearch() : Unit {
        // Investigate the success probability of the quantum search.
        ...
        // Perform Grover iterations to amplify the marked subspace.
        let nIterations = 3;

        // Number of queries to database oracle.
        let queries = nIterations * 2 + 1;

        // Execute the quantum search and verify that the success probability matches the
    ...
    operation RunMultipleQuantumSearch() : Unit {
        // Investigate the success probability of the quantum search with multiple
        // marked elements.

        // Define the size `N` = 2^n of the database to searched in terms of
        // number of qubits `n`.
        let nDatabaseQubits = 8;
        let databaseSize = 2 ^ nDatabaseQubits;

        // Define the marked elements. These must be smaller than `databaseSize`.
        let markedElements = [0, 39, 101, 234];
        let nMarkedElements = Length(markedElements);

        // Pform Grover iterations to amplify the marked subspace.
        let nIterations = 3;

        // Number of queries to database oracle.
        let queries = nIterations * 2 + 1;

        // Execute the quantum search and verify that the success
        ...
        for attempt in 1 .. repeats {
            ...
            // Determine performance improvement from the quantum algorithm performs on average
